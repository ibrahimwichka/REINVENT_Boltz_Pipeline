...
Collects Boltz-2 affinity predictions for the CDK2 known-actives calibration
set, joins them with experimental ground truth, and plots:
...
Run directly on the login node after dock_array.sh completes (fast, no GPU):
    python threshold/CDK2/analyze_calibration.py
"""
...
if __name__ == "__main__":
    ...
    ax.set_title(f"CDK2 known actives: predicted vs. experimental pIC50 (r = {r:.2f}, n = {len(results)})")
    ax.legend()
    fig.tight_layout()
    ...
    ax.set_ylabel("Count")
    ax.set_title(f"CDK2 known actives: p_bind distribution (n = {len(results)})")
    fig.tight_layout()
