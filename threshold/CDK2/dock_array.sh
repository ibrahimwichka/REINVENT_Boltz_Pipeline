#!/bin/bash
#SBATCH --job-name=cdk2_calib_dock
#SBATCH --output=threshold/CDK2/logs/dock_%A_%a.log
#SBATCH --error=threshold/CDK2/logs/dock_%A_%a.err
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
#
# Chunking: manifest.csv has 1402 CDK2 actives, CHUNK_SIZE=50 -> 29 tasks (array indices 0-28).
...
mkdir -p threshold/CDK2/logs
mkdir -p threshold/CDK2/boltz_outputs

CHUNK_SIZE=50
START=$(( SLURM_ARRAY_TASK_ID * CHUNK_SIZE ))
END=$(( START + CHUNK_SIZE ))

echo "Docking CDK2 calibration set, manifest rows $START-$END..."
python threshold/CDK2/dock_worker.py --start $START --end $END
